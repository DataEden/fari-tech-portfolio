...
import re, os # the re module for regular expression operations and os module for environment variables
from dotenv import load_dotenv
from pathlib import Path

# Load .env from current file folder OR project root
...
model_name = os.getenv("OPENAI_BASE_MODEL")
...
def get_sentiment(text: list) -> list:
    ...
    if not model_name:
        raise ValueError("Missing OPENAI_BASE_MODEL in .env")
    
    #client = OpenAI(api_key=api_key) # Create a object of the OpenAI class.
    ...
    user_prompt = (
        "Hi! It is May. Please label each review below as either positive, neutral, negative, or irrelevant. \
            And Please return exactly one label per review.: \n\n"
        f"{joined_reviews}"
    )
